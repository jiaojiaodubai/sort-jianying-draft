...
# TODO:公共资源（素材、函数）放到单独的utils.py中
...
ttps://stackoverflow.com/questions/4611237/double-null-terminated-string
'''
...
class PathX:
    ...
    def __init__(self, m: str, n: str, d: str):
        self.mould = m
        self.name = n
        self.display = d
        self.content = []
    ...
    def load(self, parser: ConfigParser, d: str = r'.\config.ini'):
        if exists(d):
            ...
            if parser.has_option(self.mould, self.name):
                self.content = parser.get('paths', self.name).split(',')
        else:
            parser.add(self.mould)
            parser.write(open(d, 'w', encoding='utf-8'))


class ParserX(ConfigParser):
    def __init__(self):
        super().__init__()

    def add_path(self, p: PathX, d: str = r'.\config.ini', update: bool = False):
        self.set(p.mould, p.name, ','.join(p.content))
        if update:
            with open(d, 'w', encoding='utf-8') as f:
                self.write(f)


# TODO: 将检查、创建、初次读取.ini文件的业务作为初始化任务的一部分转移到主程序JianyingAssistant中
class PathManager:
    install_path = PathX('public', 'install_path', '安装路径')
    draft_path = PathX('public', 'draft_path', '草稿路径')
    Data_path = PathX('public', 'install_path', 'Data路径')
    paths = []
    ...
    def __init__(self):
        ...
        try:
            ...
            exit(1)
        self.paths = [self.install_path, self.draft_path, self.Data_path]
        ...
        if exists(r'.\config.ini'):
            ...
            for i in range(end):
                if self.configer.has_option('public', self.paths[i].name):
                    self.paths[i].content = self.configer.get('paths', self.paths[i].name).split(',')
                elif i < 3:
                    return False
            return True
        else:
            ...
            self.configer.add_section('exvoice_setting')
            self.configer.add_section('extittle_setting')
